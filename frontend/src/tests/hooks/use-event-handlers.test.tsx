...
describe('useEventHandlers hook', () => {
	...
	beforeEach(async () => {
		...
		featurePopupMocks = (await import('@/components/city-map/hooks/use-feature-popup') as unknown as MockFeaturePopupModule).__mocks;
		hooksMocks = (await import('@/hooks') as unknown as MockHooksModule).__mocks;
		mapMocks = (await import('react-map-gl/maplibre') as unknown as MockMapLibreModule).__mocks;
		atomsMocks = (await import('@/atoms') as unknown as MockAtomsModule).__mocks;
	});
	...
	it('handles click event and opens popup when not selecting point', async () => {
		...
		// Assert
		await waitFor(() => {
			expect(featurePopupMocks.mockSetLngLat).toHaveBeenCalledWith(mockEvent.lngLat);
			expect(featurePopupMocks.mockSetIsPopupOpen).toHaveBeenCalledWith(true);
			expect(featurePopupMocks.mockSetProperties).toHaveBeenCalledWith(featureProperties);
	...
	it('handles click event and calls handleAddressName when selecting point', async () => {
		...
		// Assert
		await waitFor(() => {
			expect(hooksMocks.mockHandleAddressName).toHaveBeenCalledWith(mockEvent.lngLat);
