 ...
 */
export function generateCityDataParams({
  maxDistance,
  isAmenityOn,
}: {
  maxDistance: MaxDistance;
  isAmenityOn: IsAmenityOn;
}) {
  return {
 ...
 *
 * @param {Error} error - The error object to get the message from.
 * @returns {string} The error message.
 */
export const getErrorMessage = (error: Error | null) => {
  if (!error) return "An error occurred while analyzing apartments.";
